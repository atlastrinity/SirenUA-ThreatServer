...
class SingleThreat:
    ...
    def __init__(self, level: str, threat_type: Optional[str] = None,
        ...
        self.origin_latitude: Optional[float] = origin_latitude
        self.origin_longitude: Optional[float] = origin_longitude
...
class MockThreatManager:
    ...
    def set_alarm_active(self, region: str, is_active: bool) -> bool:
        ...
        if not self.threats[region].is_test:
            was_active = self.threats[region].is_active
            has_changed = (was_active != is_active)
    ...
    def get_stale_threats_without_alarm(self) -> dict:
        """Повертає список областей з активними об'єктами загроз, де офіційна сирена ВИМКНЕНА."""
        stale = {}
        for region, state in self.threats.items():
            if not state.is_active and state.active_threats:
                stale[region] = state.to_dict()
        return stale
