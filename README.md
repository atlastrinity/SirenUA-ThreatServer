...
Фоновий процес **`GeminiRulesLearner`** прокидається **кожні 6 годин** (або за ручним запитом `/api/analytics/predictions/learn`) і аналізує завершені сесії `paired_events` за останні 30 днів за **7 суворими математичними критеріями**:

1. **🗺️ Маршрутні патерни (`route_pattern`)**:
   - **Критерій**: не менше 5 переходів загрози з області А в область Б за 30 днів (`occurrence_count >= 5`). Включає як стратегічні, так і тактичні засоби (`shahed`, `reactive_uav`, `cruise_missile`, `ballistic`, `tu95`, `tu22m3`, `su35_su57`, `mig31k`, `kab`, `artillery`, `mlrs`, `fpv`, `recon`).
...
5. **✈️ Патерни авіаударів (`aviation_strike_pattern`)**:
   - **Критерій**: не менше 2 підтверджених авіаударів (`occurrence_count >= 2`) з прив'язкою авіабаз зльоту та пускових рубежів до цільових областей України.
   - **Результат**: правило миттєвої ідентифікації напрямку удару при фіксації зльоту тактичної/стратегічної авіації.

6. **🚀 Патерни пускових майданчиків (`launch_site_pattern`)**:
   - **Критерій**: не менше 2 підтверджених запусків (`occurrence_count >= 2`) з наземних або морських майданчиків (Чауда, Приморсько-Ахтарськ, Тарханкут, Бєлгород).
   - **Результат**: правило автоматичного визначення вхідного прикордонного коридору.

7. **📉 Згасання та очищення застарілих правил (`apply_rule_decay`)**:
...
#### 4. Динамічний контекстний фільтр правил (Dynamic Rules RAG & Keyword Boosting)

Для скорочення часу відповіді Gemini до субсекундного рівня (<0.8с) без перевантаження промпту зайвими токенами, функція `GeminiThreatAnalyzer.build_rules_context(messages_text)` здійснює гібридну семантичну фільтрацію:
- **Кластерне групування**: визначає активні географічні кластери (Північний, Східний, Південний, Центральний, Західний, Стратегічна авіація).
- **Прямий буст ключових слів (`1.8×`)**: правила, які містять прямі назви згаданих областей (Київ, Харків, Одеса, Суми) або типів зброї (шахед, балістика, каб, міг-31к), отримують коефіцієнт `1.8x`.
- **Пріоритет пускових патернів (`2.5×`)**: правила типів `launch_site_pattern` та `aviation_strike_pattern` мають мультиплікатор `2.5x` для негайного врахування нових позицій ворога.
- Відбирає лише 6–8 найбільш релевантних правил із найвищим рангом `(accuracy_score * evidence_count)`.

#### 5. Блокування транзитного прогнозування для тактичного озброєння

Для коротко дистанційних засобів ураження (`kab`, `artillery`, `mlrs`, `fpv`, `recon`) модуль `monitor/telegram_monitor.py` суворо обмежує радіус дії виключно прифронтовими та суміжними прикордонними областями, запобігаючи нереалістичному транзитному поширенню загрози вглиб території України.
...
- **Дворівнева тактична архітектура**:
  - *Фаза підльоту носія*: пунктирний вектор від авіабази до пускового рубежу.
  - *Фаза бойового удару*: суцільний кінетичний промінь від рубежу до цілі. Для наземних запусків (Курськ, Орел, Бєлгород) пусковий рубіж встановлюється на саму базу (0% пунктирних ліній).
...
## 🧪 Ізоляція автоматизованого тестування (Zero-Spam Testing)

- **Глушіння FCM та планувальника**: При виконанні тестів `pytest` (`PYTEST_CURRENT_TEST` або `ENV=test`), усі пуш-сповіщення та фонові таймери блокуються в пам'яті.
- **Ізольовані SQLite фікстури**: Тести використовують тимчасові незалежні бази даних, усуваючи конфлікти блокувань та змішування тестових даних з бойовими.
